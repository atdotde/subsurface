...
void vpmb_next_gradient(double deco_time, double surface_pressure)
{
	...
	double he_b, he_c;
	double desat_time;
	double n2_volume, he_volume, n2_bubble_factor, he_bubble_factor;
	deco_time /= 60.0;
	...
	for (ci = 0; ci < 16; ++ci) {
		...
		he_bubble_factor = (bottom_he_gradient[ci] - initial_he_gradient[ci])
				/ (vpmb_config.skin_compression_gammaC / vpmb_config.surface_tension_gamma * bottom_he_gradient[ci] - max_he_crushing_pressure[ci]);

		he_volume = he_bubble_factor * bottom_he_gradient[ci]
				* desat_time
				* vpmb_config.skin_compression_gammaC * vpmb_config.skin_compression_gammaC
				/ vpmb_config.surface_tension_gamma / vpmb_config.surface_tension_gamma;


		printf("%d:\tn2:%f%% %f\the:%f%% %f\n",
		       ci,
		       100.0 * n2_volume / vpmb_config.crit_volume_lambda,
		       n2_bubble_factor,
		       100.0 * he_volume / vpmb_config.crit_volume_lambda,
		       he_bubble_factor);
