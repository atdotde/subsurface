...
static void add_plan_to_notes(struct diveplan *diveplan, struct dive *dive, bool show_disclaimer, int error)
{
	...
	len = show_disclaimer ? snprintf(buffer, sz_buffer, "<div><a name=\"HEREISTHEDISCLAIMER\"> <b>%s</b><br></div>", disclaimer) : 0;

	if (diveplan->surface_interval > 60) {
		len += snprintf(buffer + len, sz_buffer - len, "<div><a name=\"HEREISTHEPLAN\"><b>%s (%s) %s %d:%02d) %s %s<br>",
		...
		len += snprintf(buffer + len, sz_buffer - len, "<div><a name=\"HEREISTHEPLAN\"><b>%s (%s) %s %s</b><br>",
				translate("gettextFromC", "Subsurface"),
				subsurface_canonical_version(),
